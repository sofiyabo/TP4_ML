...
def _binary_search_sigma(dist2_i, target_perplexity, max_iter=200, tol=1e-5):
    ...
    for _ in range(max_iter):
        ...
        # Entropía de Shannon en bits: H = -sum p * log2(p)
...
def _compute_joint_probabilities(X, perplexity):
    ...
    """
    n = X.shape[0]
    D = _pairwise_squared_distances(X)   # (n, n)

    # Matriz de probabilidades condicionales (sin diagonal)
    P_cond = np.zeros((n, n))
    for i in range(n):
        # Distancias del punto i a todos los demás (excluir i mismo)
        dist2_i = np.concatenate([D[i, :i], D[i, i+1:]])
        p_i     = _binary_search_sigma(dist2_i, perplexity)

        # Reinsertar en la fila i (con 0 en la diagonal)
        P_cond[i, :i]   = p_i[:i]
        P_cond[i, i+1:] = p_i[i:]
...
def _compute_q_matrix(Y):
    ...
    """
    n = Y.shape[0]
    D = _pairwise_squared_distances(Y)             # (n, n)
    num = 1.0 / (1.0 + D)                         # (n, n)
    np.fill_diagonal(num, 0.0)                     # p_{ii} = 0
...
def _compute_gradient(P, Q, Y, num):
    ...
    n = Y.shape[0]
    PQ = P - Q                      # (n, n)
    PQ_num = PQ * num               # (n, n) — factor escalar por par

    # grad[i] = 4 * sum_j PQ_num[i,j] * (Y[i] - Y[j])
    # Vectorizado: (n,1,2) - (1,n,2) = (n,n,2); luego sum sobre j
    diff = Y[:, None, :] - Y[None, :, :]    # (n, n, 2)
    grad = 4.0 * np.einsum('ij,ijk->ik', PQ_num, diff)   # (n, 2)
...
def tsne(X, n_components=2, perplexity=30, n_iter=1000, lr=200.0,
    ...
    if verbose:
        ...
        print("[t-SNE] Calculando probabilidades en alta dimensión...")
    ...
    for t in range(1, n_iter + 1):
        ...
        # Actualizar con momentum:  update = momentum * update - lr * grad
        ...
        if verbose and t % 100 == 0:
            # Calcular KL divergence para monitorear convergencia
